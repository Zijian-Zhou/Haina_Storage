...
function user_register(uname, pwd){
    ...
    user_mask = pwd;
    let pt = hex2buf(pwd);
    var nums = pre_init_res("./user_meta.dat", 2, pt, uname);
    Module._free(pt);
    let node = get_node();
...
function select_node(nodes_inf, nums, Beginner){
    for(let j = 0; j < nodes_nums ; j++){
        if(stored[j].name == Beginner){
            stored[j].nums++;
            break;
        }
    }
    for(let i = 0; i < nodes_inf.length; i++){
        for(let j = 0; j < nodes_nums ; j++){
            ...
            if(stored[j]["name"] == nodes_inf[i]["node"]){
                if(stored[j]["nums"] == 0 && stored[j].name != Beginner){
                    stored[j].nums++;
    ...
    for(let j = 0; j < nodes_nums ; j++){
        if(stored[j].nums != 0 && check_rate(stored[j], nums)  && stored[j].name != Beginner){
            stored[j].nums++;
            return stored[j].name;
        }
    }
...
function store_inf_reset() {
    Rate = 0.2;
    for(let i = 0;i<stored.length;i++) stored[i].nums = 0;
}
        ...
        get_check_token(url, "").then(function (check_token){
            ...
            $.ajax({
                ...
                success: function (res){
                    ...
                    let node = select_node(res, nums, Beginner);
        ...
        success: function (res){
            ...
            if(res.code == 0){
                ...
                set_messs_box("错误", "该用户不存在!");
                 ...
                 user_mask = sm3(pwd);
                 let header = sm3(uname);
                 dire_times[header] = 0;
                 get_chain(header, res.loc, user_mask);
            }
        }
    })
}
...
function hex2buf(hex){
    console.log("hex2buf ", typeof hex, hex)
    const array = new Uint8Array(hex.match(/[\da-f]{2}/gi).map(function (h) {
        return parseInt(h, 16);
    }));
    let arrayPtr = Module._malloc(array.length * 4);
...
function get_pointer_domain(path, mask){
    ...
    let pointer = hex2buf(mask);
    let ptr = get_pointers(path, pointer);
...
function clear_downloads(path="/downloads"){
...
function check_downloaded(header, block){
    for(let i = 0;i<downloads[header].length;i++){
        if(downloads[header][i] == block) return false;
    }
    return true;
}
...
function methods_switch(header, mask, method, filename, downpath){
    let re;
    if(method == "get_file"){
        add_runinf("recovery " + filename + " to " + downpath);
        re = new recovery(header, mask, false, filename, downpath)
    }else if(method == "download" || method == "delete" || method == "multi-delete"){
        traver_remove(mask, method, header, downpath);
        return;
    }else{
        re = new recovery(header, mask, true, filename, downpath)
    }
    if(!re.start()){
...
function download_block(url, block, token, header, flag, loc, mask, method, filename, downpath="/downloads"){
    add_runinf("正在下载数据块" + block);
    if(check_downloaded(header, block)){
        var data = {
            url: url,
            type: "POST",
            xhrFields: {
            ...
            success: function (data){
                ...
                reader.onload = () => {
                    var filePath = downpath + '/down_' + block + ".dat";
                    ...
                    if(flag == 1){
                        ...
                        next_dir.onmessage = e => {
                            if(e.data.code == 1){
                                download_block(e.data.url, e.data.block, e.data.token, header, 1, loc, mask, method,filename,downpath);
                            }else if(e.data.code == 2){
                            console.log("diretimes 443 ", dire_times[header]);
                            if(dire_times[header] == 1){
                                console.log("start 455");
                                dire_times[header] = 0;
                                console.log(method);
                                methods_switch(header, mask, method, filename, downpath);
                            }else{
                                dire_times[header] += 1;
                            }
                        }
                        ...
                        pre_dir.onmessage = e => {
                            if(e.data.code == 1){
                                download_block(e.data.url, e.data.block, e.data.token, header, 0, loc, mask, method, filename,downpath);
                            }else if(e.data.code == 2){
                            console.log("diretimes 461 ", dire_times[header]);
                            if(dire_times[header] == 1){
...
function get_chain(header, loc, mask, method="res", filename="./user_meta.dat", downpath="/downloads"){
    downloads[header] = [];
    dire_times[header] = 0;
    get_check_token("http://"+loc+"/download/", "").then(function (check_token){
        var data = {
            ...
            success: function (data){
                ...
                reader.onload = () => {
                    clear_downloads()
                    var filePath = downpath + '/down_' + header + ".dat";
                    Module.FS.writeFile(filePath, new Uint8Array(reader.result), {encoding: "binary"});
                    downloads[header].push(header);
                    let res = get_pointer_domain(filePath, mask);
                    ...
                    next_dir.onmessage = e => {
                        if(e.data.code == 1){
                            ...
                            console.log("diretimes 509 ", dire_times.header);
                            if(dire_times[header] == 1){
                    ...
                    pre_dir.onmessage = e => {
                        if(e.data.code == 1){
                            ...
                            console.log("diretimes 524 ", dire_times[header]);
                            if(dire_times[header] == 1){
