...
function set_out(title, mess){
    ...
    let tar = document.getElementById("mess-button");
    let temp = create_div("logout-confirm", "mess-bt");
    temp.innerText = "确认";
    temp.addEventListener("click", function(){
        location.reload();
    })
    tar.appendChild(temp);
    temp = create_div("logout-cancel", "mess-bt");
    temp.innerText = "取消";
    temp.addEventListener("click", close_search);
    tar.appendChild(temp);
    switch_dialog();
}
function init_adder(){
...
function process_data(file){
    return new Promise( (resolve) => {
        ...
        freader.onload = function (){
            ...
            let tempmess = document.getElementById("loader-inf").innerText;
            let temp_mess = document.getElementById("loader-inf").innerText;
            document.getElementById("loader-inf").innerText = "数据加密中... ...";
            var nums = pre_init(filecache_path, 10);
            document.getElementById("loader-inf").innerText = tempmess;
            document.getElementById("loader-inf").innerText = tempmess;
            let node = get_node();
            let mask = buf2hex(Module.FS.readFile('metafile.dat')).toUpperCase();
            console.log("mask up", mask);
...
async function pre_upload(uname, pwd){
    ...
    for(let i = 0; i < file_container.files.length; i++, cnt++){
        ...
        let mask = buf2hex(Module.FS.readFile('metafile.dat')).toUpperCase();
        console.log("write mask up", mask);
    ...
    let tar = document.getElementById("icon-mode");
    if(tar.className == "show-type selected"){
        build_files_view();
    }else{
        listview();
    }
...
function draw_list_title(){
    let outer, temp;
    outer = create_div("", "list-item list-title");
...
function draw_list_folder(item){
    let outer, temp, cache;
    outer = create_div("item-" + item.id, "list-item");
    temp = create_div("", "name");
    cache = create_div("", "icon");
    cache.innerHTML = "<i class=\"fa fa-folder-open\" aria-hidden=\"true\"></i>&emsp;"
    temp.appendChild(cache);
    cache = document.createElement("p");
    cache.innerText = item.name;
    temp.appendChild(cache);
    outer.appendChild(temp);
    temp = create_div("", "date");
    temp.innerText = item.last_date;
    outer.appendChild(temp);
    temp = create_div("", "size");
    temp.innerText = convert_unit(item.size);
    outer.appendChild(temp);
    temp = create_div("", "ops");
    cache = create_div("opo-item-" + item.id, "op");
    cache.innerText = "打开";
    temp.appendChild(cache);
    cache = create_div("opd-item-" + item.id, "op");
    cache.innerText = "删除";
    cache.addEventListener("click", folder_events);
    temp.appendChild(cache);
    cache = create_div("opr-item-" + item.id, "op");
    cache.addEventListener("click", folder_events);
    cache.innerText = "重命名";
    temp.appendChild(cache);
    outer.appendChild(temp);
...
function draw_list_file(item){
    let outer, temp, cache, buf;
    outer = create_div("item-" + item.id, "list-item");
    temp = create_div("", "name");
    cache = create_div("", "icon");
    buf = create_div("", "document");
    buf.innerHTML = "<i class=\"fa fa-file-text\" aria-hidden=\"true\"></i>";
    cache.appendChild(buf);
    buf = create_div("", "lock");
    buf.innerHTML = "<i class=\"fa fa-lock\" aria-hidden=\"true\"></i>";
    cache.appendChild(buf);
    temp.appendChild(cache);
    cache = document.createElement("p");
    cache.innerHTML = "&emsp;" + item.name;
    temp.appendChild(cache);
    outer.appendChild(temp);
    temp = create_div("", "date");
    temp.innerText = item.last_date;
    ...
    outer.appendChild(temp);
    temp = create_div("", "ops");
    cache = create_div("opg-item-" + item.id, "op");
    cache.innerText = "下载";
    cache.addEventListener("click", file_events);
    temp.appendChild(cache);
    cache = create_div("opd-item-" + item.id, "op");
    cache.innerText = "删除";
    cache.addEventListener("click", file_events)
    ...
    temp.appendChild(cache);
    outer.appendChild(temp);
    outer.addEventListener("dblclick", file_events);
    return outer;
}
function listview(){
    let outer = document.getElementById("show-files");
    clear_children("show-files");
    outer.appendChild(draw_list_title());
    for(let i = 0;i<struction.fs.length;i++){
        if(struction.fs[i].father == guider.cur && struction.fs[i].type == "D"){
            outer.appendChild(draw_list_folder(struction.fs[i]));
        }
    }
    for(let i = 0;i<struction.fs.length;i++){
        if(struction.fs[i].father == guider.cur && struction.fs[i].type == "F"){
            outer.appendChild(draw_list_file(struction.fs[i]));
        }
    }
    let temp = create_div("create", "list-item list-create");
    temp.innerHTML = "<i class=\"fa fa-plus-square-o\" aria-hidden=\"true\"></i>";
    outer.appendChild(temp);
    document.getElementById("create").addEventListener("click", function (){
        document.getElementById("adder").style.visibility = "visible";
    });
...
function add_options(file){
    let outer, temp;
    outer = create_div("", "header-td op-td ops");
    temp = create_div("search-item-" + file.id, "ops-op");
    if(file.type == "D"){
        temp.innerText = "打开";
        temp.addEventListener("click", folder_events);
        outer.appendChild(temp);
        temp = create_div("opd-item-" + file.id, "ops-op");
        temp.innerText = "删除";
        temp.addEventListener("click", folder_events);
    }else{
        temp.innerText = "下载";
        temp.addEventListener("click", file_events);
        outer.appendChild(temp);
        temp = create_div("opd-item-" + file.id, "ops-op");
        temp.innerText = "删除";
        temp.addEventListener("click", file_events);
    ...
    outer.appendChild(temp);
    temp = create_div("opr-item-" + file.id, "ops-op");
    temp.addEventListener("click", file_events);
    temp.innerText = "重命名";
    outer.appendChild(temp);
    return outer;
}
function add_search_item(file){
    let outer, temp, cache;
    outer = create_div("search-item-" + file.id, "results-header over");
    temp = create_div("", "header-td");
    temp.innerText = file.name;
    outer.appendChild(temp);
    temp = create_div("", "header-td op-td");
    temp.innerText = file.last_date;
    outer.appendChild(temp);
    temp = create_div("", "header-td op-td");
    temp.innerText = convert_unit(file.size);
    outer.appendChild(temp);
    outer.appendChild(add_options(file));
    return outer;
}
function search_file(){
    let key = document.getElementById("search-input").value;
    let tar = document.getElementById("results-items");
    clear_children("results-items");
    console.log(key);
    for(let i=0;i<struction.fs.length;i++){
        if(struction.fs[i].id != 0 && struction.fs[i].name.search(key) != -1){
            tar.appendChild(add_search_item(struction.fs[i]));
        }
    }
...
function search_box(){
    ...
    cache.setAttribute("id", "search-file-bt");
    cache.setAttribute("class", "fa fa-search sbt");
    cache.addEventListener("click", search_file);
    //cache.setAttribute("aria-hidden", "true");
    temp.appendChild(cache);
    outer.appendChild(temp);
    temp = create_div("search-results", "search-results");
    temp.appendChild(create_search_header());
    cache = create_div("results-items", "results-items style-1");
    temp.appendChild(cache);
    outer.appendChild(temp);
...
class recovery{
    constructor(header, mask, flag=false, defilename="./user_meta.dat", downpath="/downloads") {
        this.files = Module.FS.readdir(downpath);
        this.nums = this.files.length - 2;
        this.flag = flag;
        this.defile = defilename;
        this.downdir = downpath + "/";
...
function folder_events(e){
    ...
    e.stopPropagation();
    e.preventDefault();
    if(document.getElementById("dialog").style.visibility == "visible"){
        document.getElementById("dialog").style.visibility = "hidden";
        close_search();
    }

    if(e.type == "click" && this.id.substring(0, 4) == "item"){
        open_folder(this.id.substring(5));
    }else if(e.type == "click" && this.id.substring(0, 11) == "search-item"){
        open_folder(this.id.substring(12));
    }else if(e.type == "click" && this.id.substring(0, 3) == "opd"){
        delete_folder(this.id.substring(9));
    }else if(e.type == "click" && this.id.substring(0, 3) == "opr"){
        let idn = this.id.substring(9);
        ready_rename(idn);
    }
}
function file_events(e){
    ...
    if(e.type == "click" && this.id.substring(0, 3) == "opd"){
        let idn = this.id.substring(9);
        ready_del_file(idn);
        ...
        let idn = this.id.substring(9);
        ready_rename(idn);
    }else if(e.type == "dblclick" || (e.type == "click" && (this.id.substring(0, 3) == "opg" || this.id.substring(0, 11) == "search-item"))){
        let idn = null;
        let file_inf = null;
        if(e.type == "dblclick"){
            idn = this.id.substring(5);
        }else if(e.type == "click" && this.id.substring(0, 3) == "opg"){
            idn = this.id.substring(9);
        }else if(e.type == "click" && this.id.substring(0, 11) == "search-item"){
...
function build_files_view(flag=true){
    let outer = document.getElementById("show-files");
    clear_children("show-files");
    for(let i = 0;i<struction.fs.length;i++){
        if(struction.fs[i].father == guider.cur && struction.fs[i].type == "D"){
...
function regenerate_user_meta(){
    return new Promise((resolve) => {
        ...
        let pt = hex2buf(user_mask);
        var nums = pre_init_res("source.dat", 2, pt, struction.username);
        Module._free(pt);
        let node = get_node();
...
async function traver_remove(mask, method, header, downpath="/downloads"){
    return new Promise(async resolve => {
        add_runinf("method" + method + " " + header + " to path: " + downpath)
        let files = Module.FS.readdir(downpath);
        for(let i = 0;i<files.length;i++){
            if(files[i] != ".." && files[i] != "."){
        ...
        if(method == "delete" || method == "multi-delete"){
            for(let i = 0;i<struction.fs.length;i++){
                if(struction.fs[i].first == header){
                    struction.size -= struction.fs[i].size;
                    add_runinf("deleting " + i + " from STRUCTION...");
                    struction.fs.splice(i, 1);
                    break;
                }
            }
        }
        document.getElementById("loader-inf").innerText = "正在重载用户Meta... ...";
        if(method == "delete" || method == "multi-delete"){
            redraw_chart(guider.cur);
            document.getElementById("loader-inf").innerText = "删除完成!";
            document.getElementById("loader").style.visibility = "hidden";
            let tar = document.getElementById("icon-mode");
            if(tar.className == "show-type selected"){
                build_files_view();
    ...
    $.ajax({
        ...
        success: function (res){
            ...
            if(res.code == 0){
                ...
                let header = sm3(struction.username);
                dire_times[header] = 0;
                get_chain(header, res.loc, user_mask, "download");
            }
        }
    })
}
function rm_file(filehash,filemask,update){
    return new Promise(resolve => {
        $.ajax({
            url: "/checkblock/",
            type: "POST",
            data: {
                "data": JSON.stringify({"block": filehash, "code": 1}),
                "csrfmiddlewaretoken": document.getElementsByName("csrfmiddlewaretoken")[0].value,
            },
            success: async function (res){
                res = JSON.parse(res);
                ...
                if(res.code == 0){
                    document.getElementById("loader").style.visibility = "hidden";
                    add_runinf("文件Meta不存在!删除失败!");
                    set_messs_box("错误", "文件Meta不存在!删除失败!");
                    resolve();
                }else if(res.code == 1){
                    dire_times[filehash] = 0;
                    if(update){
                        await get_chain(filehash, res.loc, filemask, "multi-delete", "./temp_download.dat", "/"+filehash);
                        resolve();
                    }else{
                        await get_chain(filehash, res.loc, filemask, "delete", "./temp_download.dat");
                        resolve();
                    }
                }
            }
...
function rename_item(){
    ...
    document.getElementById("loader-inf").innerText = "正在进行重命名操作... ...";
    document.getElementById("loader").style.visibility = "visible";
    for(let i = 0;i<struction.fs.length;i++){
        if(id == struction.fs[i].id){
            flag = true;
            item = struction.fs[i];
            index = i;
        }
    ...
    if(flag){
        ...
        for(let i = 0;pass && i<struction.fs.length;i++){
            if(struction.fs[i].father == item.father && struction.fs[i].name == newname && struction.fs[i].type == item.type){
                pass = false;
                code = 2;
            }
        }
        if(pass){
            struction.fs[index].name = newname;
            let tar = document.getElementById("icon-mode");
            if(tar.className == "show-type selected"){
                build_files_view();
            }else{
            ...
            document.getElementById("loader").style.visibility = "hidden";
            add_runinf("当前文件夹该名称已存在相同名称或文件（夹）未更改名称!\ncode:EXISTED-" + code);
            set_messs_box("错误", "当前文件夹该名称已存在相同名称或文件（夹）未更改名称!\ncode:EXISTED-" + code);
        }
    }
    if(!flag){
        document.getElementById("loader").style.visibility = "hidden";
        add_runinf("所操作对象不存在，code: OPR-OBJ_NOT_FOUND-2!");
        set_messs_box("错误", "所操作对象不存在，code: OPR-OBJ_NOT_FOUND-2!");
    }
}
function ready_rename(id){
    let item;
    let flag = false;
    for(let i = 0;i<struction.fs.length;i++){
        if(id == struction.fs[i].id){
            item = struction.fs[i];
            flag = true;
            ren_obj = struction.fs[i].id;
        }
    }
    if(flag = true){
        document.getElementById("rename-box").style.visibility = "visible";
        document.getElementById("new-name-input").value = item.name;
    }else{
        add_runinf("所操作对象不存在，code: OPR-OBJ_NOT_FOUND!");
        set_messs_box("错误", "所操作对象不存在，code: OPR-OBJ_NOT_FOUND!");
    }
}
function init_rename_box(){
    let outer, buf, temp, cache;
    ren_obj = 0;
    outer = create_div("rename-box", "adder");
    buf = create_div("", "box");
    temp = create_div("", "title");
    temp.innerHTML = "请&emsp;输&emsp;入&emsp;新&emsp;名&emsp;称";
    buf.appendChild(temp);
    temp = create_button("new-name-input", "", "请输入新名称", "text");
    buf.appendChild(temp);
    temp = create_div("", "buts");
    cache = create_div("change_name_confirm", "bt");
    cache.innerHTML = "确&emsp;认";
    cache.addEventListener("click", rename_item);
    temp.appendChild(cache);
    cache = create_div("cancle-rename", "bt");
    cache.innerHTML = "取&emsp;消";
    cache.addEventListener("click", cancle_rename);
    temp.appendChild(cache);
    buf.appendChild(temp);
...
function  cancle_rename(){
    document.getElementById("rename-box").style.visibility = "hidden";
}
function ready_del_file(idn, update=false){
    return new Promise(resolve => {
        let hash = null;
        let mask = null;
        let file = null;
        ...
        for(let i = 0;i<struction.fs.length;i++){
            if(struction.fs[i].id == idn){
                hash = struction.fs[i].first;
                mask = struction.fs[i].mask;
                file = struction.fs[i];
                break;
            }
        }
        if(hash != null && mask != null){
            document.getElementById("loader-inf").innerText = "正在删除文件" + file.name + "... ...";
            document.getElementById("loader").style.visibility = "visible";
            rm_file(hash, mask, update).then(() => {
                resolve();
            })
        }else{
            add_runinf("所删数据映射发生错误!\n文件ID:"+idn);
            set_messs_box("错误", "所删数据映射发生错误!\n文件ID:"+idn)
        }
    })
}
function delete_folder(idn, update=true){
    return new Promise(async resolve => {
        console.log(idn);
        for(let i = 0;i<struction.fs.length;i++){
            if(struction.fs[i].father == idn){
                if(struction.fs[i].type == "D"){
                    delete_folder(struction.fs[i].id, false).then(() => {

                    })
                }else{
                    Module.FS.mkdir("/" + struction.fs[i].first);
                    ready_del_file(struction.fs[i].id, true).then(() => {
